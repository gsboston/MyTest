<template>
    <div>
        <Row type="flex">
            ...
            <i-col span="9" offset="2">
                <ul class="personsWrap">
                    <li :class="{active: personsIdActive === item.id}" v-for="(item, index) in persons" @click="personsItemHandle(item.id, item)">
        ...
        <slot name="btn">
            <div class="btnWrap">
                <Button type="primary" @click="save" :disabled="!orgIdActive || !personsIdActive">保存</Button>
            </div>
        </slot>
    </div>
</template>
