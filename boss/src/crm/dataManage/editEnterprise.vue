    ...
    <div>
        ...
        <Row>
            <i-Col span="12">
                ...
                <Form class="basic-info-wrap basic-info-wrap-line" ref="enterprise" :model="enterprise" :rules="ruleValidate" :label-width="80">
                    <Row>
                        <i-Col span="4">
            ...
            <i-Col span="12">
                ...
                <Form v-if="relatedHintsList && relatedHintsList.length > 0" class="basic-info-wrap" ref="personal" :model="personal" :rules="ruleValidRelated" :label-width="80">
                    <Row>
                        <i-Col span="4">
                            ...
                            <Row>
                                <i-Col span="8">
                                    <FormItem label="省份" prop="provinceCode" :rules="{required: true, message: '省份不能为空', trigger: 'change'}">
                                        <Select v-model="personal.provinceCode" filterable @on-change="provinceRelatedChange">
                                ...
                                <i-Col span="8">
                                    <FormItem label="城市" :label-width="50" prop="cityCode" :rules="{required: true, message: '城市不能为空', trigger: 'change'}">
                                        <Select v-model="personal.cityCode" filterable @on-change="cityRelatedChange">
...
<script>
    ...
    export default {
        ...
        methods: {
            ...
            getDetails() {
                this.request({
                    url: '/crm/dataManage/enterpriseDetails.do',
                    param: {
                        hxId: this.clueId
                    }
                }).then(data => {
                    this.mainClue = data.mainClue;
                    this.handleEnterpriseData(this.mainClue.company);
            ...
            changeRelatedHints(item){
                if(this.relatedHintsState != item.id){
                    ...
                    this.isShowRelatedHintsMenu = false;
                    this.handlePersonalData (item.value.personal, item.value.tags);
                }
            },
            /*选择线索*/
            selectRelatedHints(item){
                this.$refs["personal"].validate((valid) => {
                    if (valid && !this.invalidPersonalMobile) {
                        ...
                        this.$Message.error('联系人信息表单验证失败!');
                    }
                })
            },
            /*处理个人后台返回的数据*/
                        ...
                        if (valid && !this.invalidPersonalMobile) {
                            this.$refs["enterprise"].validate((valid) => {
                                if (valid && !this.invalidEnterpriseName) {
                                    this.request({
                                        url: '/crm/dataManage/saveEnterprise.do',
                                        data: {
