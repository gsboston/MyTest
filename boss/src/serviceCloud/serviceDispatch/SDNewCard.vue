    ...
    <div style="padding: 10px 15px;">
        <Row>
            <Form class="invoice-form" ref="form" :model="serviceForm" label-position="left" :label-width="120" :rules="ruleValidate">
                <i-col span="16" offset="8" style="padding: 10px 0 20px;">
                    <Steps :current="currentStep">
                        <Step title="生成服务单"></Step>
                        <Step title="派单"></Step>
                    </Steps>
                </i-col>

                <template v-if="!isOnlyDispatch">
                    <i-col span="24">
                    ...
                    <i-col span="23">
                        ...
                        <i-col span="17">
                            <FormItem ref="managerUserId" class="s-price" label="客户经理" prop="managerUserId">
                                <Select filterable v-model="managerUserIndex" @on-change="managerUserChange">
                ...
                <i-col span="24" class="saveBtn" v-if="!isOnlyDispatch">
                    <i-col span="6" offset="4">
                        <Button type="ghost" @click="openServiceTips('NONE_DISPATCH', 'create')">{{ templeteData.isUnPack ? '仅拆分服务单' : '仅生成服务单' }}</Button>
                    </i-col>
                    <i-col span="6" offset="4">
                        <Button type="primary" @click="openServiceTips('NONE_DISPATCH', 'createToDispatch')">{{ templeteData.isUnPack ? '拆分并派单' : '生成并派单' }}</Button>
                    </i-col>
                </i-col>

                <i-col span="24" class="saveBtn" v-else>
                    <i-col span="6" offset="4">
                        <Button type="ghost" @click="cancel">暂不派单</Button>
                    </i-col>
                    <i-col span="6" offset="4">
                        <Button type="primary" @click="openServiceTips('ALREADY_DISPATCH', 'dispatch')">确认派单</Button>
                    </i-col>
                </i-col>
            </Form>
        </Row>
        ...
        <Modal v-model="serviceTips" width="360">
            ...
            <div slot="footer" style="text-align: center;">
                ...
                <Button type="primary" size="large" @click="save(saveType)">{{ serviceForm.state == 'ALREADY_DISPATCH' ? '继续派单' : '继续生成' }}</Button>
            </div>
        </Modal>
    </div>
</template>
...
<script>
    ...
    export default {
        ...
        methods: {
                ...
                this.request({
                    ...
                    if (data) {
                        data.forEach((item, index) => {
                            if (item.id == this.contractInfo.saleUserId) {
                ...
                if ( this.templeteData.isUnPack ) {
                    ...
                    if (oneForm && twoForm) {
                        this.request({
                            ...
                            this.templeteData.isUnPack = false;
                            this.serviceForm.templeteData = JSON.stringify(this.templeteData.serviceInfoOne);
                            this.handleSaveSuccess(type, dataOne, '拆分服务单成功', '拆单并派单成功');
                        });
                    } else if (oneForm) {
                        this.request({
                            url: "/serviceCloud/serviceDispatch/save.do",
                            ...
                            this.templeteData.isUnPack = false;
                            this.serviceForm.templeteData = JSON.stringify(this.templeteData.serviceInfoOne);
                            this.handleSaveSuccess(type, data, '拆分服务单成功', '拆单并派单成功');
                        });
                    } else if (twoForm) {
                        this.request({
                            url: "/serviceCloud/serviceDispatch/save.do",
                    ...
                    if (type == 'create' || type == 'createToDispatch') {
                    ...
                    this.request({
                        ...
                        this.handleSaveSuccess(type, data, '服务单生成成功', '服务单派单成功');
                    });
                }
            },
            openServiceTips(status, type) {
            ...
            /**
             * 生成服务单/派单成功后执行方法
             * @param type 操作类型 [createToDispatch: '生成并派单', create: '仅生成服务单', dispatch: '派单']
             * @param data 返回服务单ID
             * @param createTitle 生成服务单成功提示语
             * @param dispatchTitle 派单成功提示语
             */
            handleSaveSuccess(type, data, createTitle, dispatchTitle) {
                if (type == 'createToDispatch') {
                    this.serviceForm.id = data;
                    this.serviceForm.serverList[0].secBillId = data;
                    this.currentStep = 1;
                    this.isOnlyDispatch = true;
                }else if (type == 'create') {
                    this.$Notice.success({title: createTitle});
                    this.callback(true, 'create');
                }else {
                    this.$Notice.success({title: dispatchTitle});
            ...
            getOrgServePostList () {
                this.request({
                    ...
                    param: {
                        billDefId: this.serviceForm.billDefId
                    }
                }).then((data)=>{
...
<style scoped>
    ...
    .invoice-form >>> .ivu-steps-title {
        display: block;
        width: 100px;
        text-align: center;
        margin: 5px 0 4px -32px;
    }
    .invoice-form .ivu-steps-item.ivu-steps-status-finish >>> .ivu-steps-head-inner span, .invoice-form .ivu-steps-item.ivu-steps-status-finish >>> .ivu-steps-head-inner>.ivu-steps-icon {
        color: #FFF;
    }

    .invoice-form .ivu-steps-item.ivu-steps-status-process >>> .ivu-steps-head-inner, .invoice-form .ivu-steps-item.ivu-steps-status-finish >>> .ivu-steps-head-inner {
        border-color: #54B499;
        background-color: #54B499;
    }

    .invoice-form .ivu-steps-item.ivu-steps-status-finish >>> .ivu-steps-tail>i:after {
        background: #54B499;
    }

    .invoice-form .ivu-steps >>> .ivu-steps-tail {
        top: 11px;
    }

    .invoice-form .ivu-steps >>> .ivu-steps-tail>i {
        height: 4px;
    }
